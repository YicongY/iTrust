...
//								"SELECT * FROM appointment WHERE doctor_id=? AND sched_date > NOW() ORDER BY sched_date;")
								"SELECT * FROM appointment WHERE doctor_id=? AND sched_date > NOW() ORDER BY sched_date;")
						: conn.prepareStatement(
								"SELECT * FROM appointment WHERE patient_id=? ORDER BY sched_date;")) {
			stmt.setLong(1, mid);
	...
	// uc807 modification
	public void updateIsRated(final int appt_id) throws DBException{
        try (Connection conn = factory.getConnection();
             PreparedStatement stmt = conn
                     .prepareStatement("UPDATE appointment SET isRated=? WHERE appt_id=?")) {
            stmt.setBoolean(1, true);
            stmt.setInt(2, appt_id);
            stmt.executeUpdate();
        } catch (SQLException e) {
            throw new DBException(e);
        }
